
<section id="mapa">
    <div class="row align-items-start justify-content-center bg-color-light g-4">
        <div class="col-12 col-lg-6 mapa pb-4">
            <h2 class="alternative-font-4 text-color-primary font-weight-bold line-height-1 text-5 mb-1">DONDE ESTAMOS</h2>
            <hr>
            <p class="font-weight-bold text-color-dark line-height-7 text-4">
                "Finca El Azusto”, Cafayate (paraje Chimpa)
            </p>
            <p class="font-weight-bold text-color-dark line-height-1 mb-1 text-3">
                Provincia de Salta, Argentina
            </p>
            <hr>
            <p class="font-weight-light text-color-dark line-height-1">
                <strong class="font-weight-bold">Reservas grupales:</strong>  info@astrapia.com.ar
            </p>
            <p class="font-weight-light text-color-dark line-height-1">
                <strong class="font-weight-bold">Reclamos o Recomendaciones:</strong>  info@astrapia.com.ar
            </p>
            <p class="font-weight-light text-color-dark line-height-1">
                <strong class="font-weight-bold">Whatsapp:</strong>  (3872) 580-115
            </p>
            <hr>
            <h2 class="alternative-font-4 text-color-primary font-weight-bold line-height-1 text-5 mb-1">CONTÁCTANOS</h2>
            <hr>
            <p class="font-weight-light text-color-dark line-height-7 mb-1">
                Para más información o reservas, envíanos un Whatsapp.
            </p>
            <p class="font-weight-light text-color-dark line-height-7">
                Click aquí abajo.
            </p>
            <a href="https://wa.me/message/UXNBRQGKYWGRL1" class="btn btn-gradient custom-btn-effect-1 custom-border-radius-1 d-inline-flex align-items-center font-weight-semibold text-3-5 btn-px-5 btn-py-3 appear-animation" data-hash data-hash-offset="0" data-hash-offset-lg="32" data-appear-animation="fadeInUpShorter" data-appear-animation-delay="1600" data-appear-animation-duration="1.7s">
                CONSULTAS O RESERVAS
            </a>
        </div>
        <div class="col-12 col-lg-5 pb-4">
            <div class="mapouter">
                <div class="gmap_canvas">
                    <iframe 
                        loading="lazy"
                        id="gmap_canvas" 
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d4469.476669025197!2d-65.86773338821185!3d-26.09741657704716!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x941f35053bcbc48d%3A0xfe13b10eb1c35531!2sGLAMPING%20ASTRAPIA!5e1!3m2!1ses!2sar!4v1774835717653!5m2!1ses!2sar" 
                        frameborder="0" 
                        scrolling="no" 
                        marginheight="0" 
                        marginwidth="0">
                    </iframe>
                    <br>
                    <a href="https://embedgooglemap.net/124/"></a>
                    <a href="https://www.embedgooglemap.net">embedgooglemap.net</a>
                </div>
            </div>
        </div>
    </div>
</section>